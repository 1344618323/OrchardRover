...
//        std::cout << " Tlc: \n"<< Tlc<<std::endl;
        ...
        cameraptr_->projectPoints(pts, r, t, pixel, true);
//        cv::projectPoints(pts, cv::Vec3d::zeros(), cv::Vec3d::zeros(), cvK_, cvD_, pixel);

        cv::Mat img_source = cv_bridge::toCvShare(image_in, "bgr8")->image;
        cv::Mat img_src;
        undistort(img_source, img_src, K, coeff);
...
int main(int argc, char **argv) {
    ...
    K = cv::Mat(3, 3, CV_64F);
    K.at<double>(0, 0) = paras.fx();
    K.at<double>(0, 1) = 0;
    K.at<double>(0, 2) = paras.cx();
    K.at<double>(1, 0) = 0;
    K.at<double>(1, 1) = paras.fy();
    K.at<double>(1, 2) = paras.cy();
    K.at<double>(2, 0) = 0;
    K.at<double>(2, 1) = 0;
    K.at<double>(2, 2) = 1;
    coeff = cv::Mat(1, 4, CV_64F);
    coeff.at<double>(0, 0) = paras.k1();
    coeff.at<double>(0, 1) = paras.k2();
    coeff.at<double>(0, 2) = paras.p1();
    coeff.at<double>(0, 3) = paras.p2();
