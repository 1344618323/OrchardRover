...
std::vector<std::vector<CamPose> >
GetStaticPose(const std::vector<CamPose> Poses, std::vector<CamPose> &avergeStaticPoses) {
    ...
    for (size_t i = 0; i < staticPoses.size(); ++i) {
        ...
        for (size_t j = 0; j < n; ++j) {
            ...
            Eigen::Vector4d qc(staticPoses[i][j].qwc.w(), staticPoses[i][j].qwc.x(), staticPoses[i][j].qwc.y(),
                               staticPoses[i][j].qwc.z());
            A_c += qc * qc.transpose();
        }
        c.twc = c_t_mea / n;
        A_c /= n;
...
void TranScanToPoints(const sensor_msgs::LaserScan &scan_in, std::vector<Eigen::Vector3d> &Points) {
    ...
    for (size_t i = 0; i < n_pts; ++i) {
        ...
        if (scan_in.range_min < range && range < range_cutoff) {
