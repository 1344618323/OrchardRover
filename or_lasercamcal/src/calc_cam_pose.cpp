...
//  cv::solvePnPRansac(p3ds, p2ds, K, dist, cv_r, cv_t, false, 100, 2., 0.99);
    ...
    cv::putText(
            img_raw, "t_wc: (m) " + std::to_string(Twc(0, 3)) + " " + std::to_string(Twc(1, 3)) + " " +
    ...
    cv::line(img_raw, imgpts[0], imgpts[1], cv::Scalar(255, 0, 0), 2);//蓝轴：w系x轴
    cv::line(img_raw, imgpts[0], imgpts[2], cv::Scalar(0, 255, 0), 2);//绿轴：w系y轴
    cv::line(img_raw, imgpts[0], imgpts[3], cv::Scalar(0, 0, 255), 2);//红轴：w系z轴
...
bool CamPoseEst::calcCamPose(const double &timestamps, const cv::Mat &image,
    ...
    //检测图像中的标定板；p3ds为特征点在标定板坐标系中坐标，z坐标都为0；p2ds为特征点在像素坐标系中坐标
    std::vector<cv::Point3f> p3ds;
    std::vector<cv::Point2f> p2ds;
