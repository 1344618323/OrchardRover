...
namespace or_costmap {
    void ObstacleLayer::OnInitialize() {
        ...
        //observation_buffers_,marking_buffers_,clearing_buffers_这三个指向的是同一堆变量
        ...
        std::shared_ptr<message_filters::Subscriber<sensor_msgs::LaserScan>> sub(
                new message_filters::Subscriber<sensor_msgs::LaserScan>(nh, topic_string, 50));
        std::shared_ptr<tf::MessageFilter<sensor_msgs::LaserScan>> filter(
    ...
    void ObstacleLayer::LaserScanCallback(const sensor_msgs::LaserScanConstPtr &message,
        ...
        //将点云从sensor_msgs::LaserScan转成sensor_msgs::PointCloud2格式，
        //sensor_msgs::PointCloud2格式的点云在代码`buffer->BufferCloud(temp_cloud)`会转换成pcl格式的点云
        //pcl这种格式的点云，优势在于其还会存储 雷达原点坐标
        //因此，障碍物层能保留多次测量的结果
    ...
    bool ObstacleLayer::GetMarkingObservations(std::vector<Observation> &marking_observations) const {
        ...
        // 获取最近的一些观测A，若数据不够新，则返回false，否则返回true
        // 观测A加上固定观测B，输出A+B
    ...
    //每次输入点云后（得到一堆射线），找到这些射线的起点、终点（若起点、终点超出grid的范围，则将这些起点、终点修改到grid的边框）
    //将这些射线的起点到终点经过的cell填充为0值，并得到一个能包含这些射线的boundingBox
    ...
    //对每条射线都求一个能包含住它（就是它的起点、终点）的boundingBox
    ...
    // 将射线经过的cell填为0，将射线hit到的cell填为254(若hit到的cell距离射线起点太远，如超过5m；或hit到的cell在grid之外，就不管了)，
    // 最后得到一个能包含所有被赋值cell的、且包含机器人多边形的boundingBox
    ...
    void ObstacleLayer::UpdateCosts(Costmap2D &master_grid, int min_i, int min_j, int max_i, int max_j) {
        ...
        if (footprint_clearing_enabled_) {
            //如果footprint_clearing_enabled_设置为true，那将多边形内的cell置为0；
            //防止落在车内的激光点被识别为障碍物
